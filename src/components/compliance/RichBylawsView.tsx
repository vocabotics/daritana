...
export const RichBylawsView: React.FC = () => {
  ...
  return (
    <div className="min-h-screen bg-gradient-to-br from-slate-50 to-blue-50">
      <div className="space-y-8 p-6">
        ...
        >
          {[
            { icon: BookOpen, label: 'Total By-laws', value: TOTAL_BYLAWS, color: 'blue' },
            { icon: AlertTriangle, label: 'Critical', value: getCriticalBylaws().length, color: 'red' },
            { icon: Flame, label: 'Fire Safety', value: getBylawsByCategory('fire_safety').length, color: 'orange' },
            { icon: Building, label: 'Structural', value: getBylawsByCategory('structural').length, color: 'green' }
          ].map((stat, index) => (
            <motion.div
              key={stat.label}
              initial={{ opacity: 0, y: 20 }}
