...
import { Project, Quotation, QuotationItem } from '@/types';
import {
  Dialog,
  DialogContent,
  DialogDescription,
...
export default function QuotationForm({ onClose, projects, quotation }: QuotationFormProps) {
  const { addQuotation, updateQuotation } = useFinancialStore();
  const [formData, setFormData] = useState<QuotationFormData>({
    ...
    validUntil: quotation ? new Date(quotation.validUntil).toISOString().split('T')[0] : '',
  ...
  const handleSubmit = (e: React.FormEvent) => {
    ...
    if (!formData.projectId || !formData.contractorId || !formData.title || formData.items.length === 0) {
    ...
    const quotationData: Quotation = {
      ...
      submittedAt: quotation?.submittedAt || new Date()
    };
    ...
    <Dialog open={true} onOpenChange={onClose}>
      <DialogContent className="sm:max-w-4xl max-h-[90vh] overflow-y-auto">
        <DialogHeader>
          ...
          <DialogDescription>
            {quotation ? 'Update the quotation details below.' : 'Fill in the details to create a new quotation.'}
          </DialogDescription>
        </DialogHeader>
          ...
          <div className="grid grid-cols-2 gap-4">
            ...
            <div className="space-y-2">
              ...
              <Input
                id="validUntil"
                type="date"
                value={formData.validUntil || defaultValidUntil.toISOString().split('T')[0]}
